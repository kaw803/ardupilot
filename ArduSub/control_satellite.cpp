...
bool Sub::satellite_init()
{
    ...
    gcs().send_text(MAV_SEVERITY_CRITICAL,"hello SATELLITE mode");
    return true;
}

void Sub::satellite_run()
{
    static float follow_yaw = 0.0f;           // 親SUBとの位置関係 -90～0[deg]:右後ろ、0～90[deg]：左後ろ（0：真後ろ、180：真ん前）
    float follow_distance = 5.0f;       // 親SUBとの距離[m]

    // 目指すポイント
    float target_north = FM_master_north - follow_distance * cosf( deg2rad(follow_yaw) );
    float target_east  = FM_master_east  - follow_distance * sinf( deg2rad(follow_yaw) );
    ...
    follow_yaw = adj360( follow_yaw + 0.05 );
}
