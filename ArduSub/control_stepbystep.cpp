...
bool Sub::sbsFollow_init()
{
    ...
    gcs().send_text(MAV_SEVERITY_CRITICAL,"hello RIGHTSIDE mode");
    return true;
}
void Sub::sbsFollow_run()
{
    float follow_yaw = 0.0f;          // 親SUBとの位置関係 -90～0[deg]:右後ろ、0～90[deg]：左後ろ（0：真後ろ、180：真ん前）
    float follow_distance = 15.0f;      // 親SUBとの距離[m]
